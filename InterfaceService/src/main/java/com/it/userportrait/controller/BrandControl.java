...
public class BrandControl {
    ...
    @RequestMapping(value = "brandData",method = RequestMethod.POST)
    public CurveEntity curveData(){

        String sql = "select brand,timeinfoString,sum(numbers) as nubmers from brandbyinfo group by brand,timeinfoString order by timeinfoString";
        ...
        try {
            ...
            while(resultSet.next()){
                ...
                Map<String,Long> datainnerMap = dataMap.get(brand);
                ...
                dataMap.put(brand,datainnerMap);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
