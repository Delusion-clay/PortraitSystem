...
@RequestMapping("api")
@CrossOrigin
public class ProductyTypeControl {

    @RequestMapping(value = "productTypeData",method = RequestMethod.POST)
    public CurveEntity curveData(){
        //最近一小时
        String sql = "select productTypeid,timeinfoString,sum(numbers) as nubmers from productTypeAnalyinfo group by productTypeid,timeinfoString order by timeinfoString";
        ...
        try {
            ...
            while(resultSet.next()){
                ...
                Map<String,Long> datainnerMap = dataMap.get(productTypeid);
                ...
                dataMap.put(productTypeid,datainnerMap);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        for(Map.Entry<String,Map<String,Long>> entry:dataMapSet){
            ...
            try {
                name = HbaseUtils2.getdata("produttype",name,"info","producttypename");
            } catch (Exception e) {
                e.printStackTrace();
            }
