...
public class ConsumerControl {
    ...
    @RequestMapping(value = "consumerData",method = RequestMethod.POST)
    public CurveEntity curveData(){

        String sql = "select consumeFlag,timeinfoString,sum(numbers) as nubmers from consumerFenbuinfo group by consumeFlag,timeinfoString order by timeinfoString";
        ...
        try {
            ...
            while(resultSet.next()){
                String consumeFlag = resultSet.getString("consumeFlag");
                String timeinfoString = resultSet.getString("timeinfoString");
                long numbers = resultSet.getLong("nubmers");
                dateSet.add(timeinfoString);
                consumeFlag = dictmap.get(consumeFlag);
                Map<String,Long> datainnerMap = dataMap.get(consumeFlag);
                ...
                dataMap.put(consumeFlag,datainnerMap);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
