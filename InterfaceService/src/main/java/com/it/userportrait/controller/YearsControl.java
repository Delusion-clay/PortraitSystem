...
@RequestMapping("api")
@CrossOrigin
public class YearsControl {

    @RequestMapping(value = "yearsData",method = RequestMethod.POST)
    public CurveEntity curveData(){
        //最近一小时
        String sql = "select yearslabel,timeinfoString,sum(numbers) as nubmers from yearslableinfo group by yearslabel,timeinfoString order by timeinfoString";
        ...
        try {
            ...
            while(resultSet.next()){
                ...
                dataMap.put(yearslabel,datainnerMap);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
